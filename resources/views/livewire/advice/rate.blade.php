<div class="max-w-md mx-auto p-6 bg-white shadow rounded-xl mt-6">
    ...
    <form wire:submit.prevent="submit" class="space-y-4">
            ...
            @for ($i = 1; $i <= 5; $i++)
                <button type="button" wire:click="$set('rating', {{ $i }})" class="focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 24 24"
                         fill="currentColor"
                         class="w-8 h-8 transition-all duration-200 ease-in-out {{ $rating >= $i ? 'text-yellow-400' : 'text-gray-300' }}">
        ...
        <button type="submit" class="w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700" wire:loading.attr="disabled">
            Gửi đánh giá
        </button>
    </form>
</div>
